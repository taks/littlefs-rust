    ...
    for n in [5usize, 25] {
        ...
        let result = run_powerloss_linear(
            ...
            |lfs_ptr, config| {
                ...
                for i in 0..n {
                    let path = path_bytes(&format!("hi{i:03}"));
                    if lfs_file_open(lfs_ptr, file, path, LFS_O_CREAT | LFS_O_WRONLY)
                        .is_err()
                    {
                        return Err(Error::Invalid);
                    }
...
fn test_dirs_nested() {
    ...
    assert_ok(lfs_rename(
        lfs,
        c"potato",
        path_bytes("coldpotato"),
    ));
    assert_ok(lfs_rename(
        lfs,
        path_bytes("coldpotato"),
        path_bytes("warmpotato"),
    ));
    assert_ok(lfs_rename(
        lfs,
        path_bytes("warmpotato"),
        path_bytes("hotpotato"),
    ));

    assert_err(Error::NoEntry, lfs_remove(lfs, c"potato"));
    assert_err(
        Error::NoEntry,
        lfs_remove(lfs, path_bytes("coldpotato")),
    );
    assert_err(
        Error::NoEntry,
        lfs_remove(lfs, path_bytes("warmpotato")),
    );
    assert_err(
        Error::NotEmpty,
        lfs_remove(lfs, path_bytes("hotpotato")),
    );

    assert_ok(lfs_mkdir(lfs, c"coldpotato"));
    assert_ok(lfs_rename(
        lfs,
        path_bytes("hotpotato/baked"),
        path_bytes("coldpotato/baked"),
    ));
    assert_ok(lfs_rename(
        lfs,
        path_bytes("hotpotato/fried"),
        path_bytes("coldpotato/fried"),
    ));
    assert_ok(lfs_rename(
        lfs,
        path_bytes("hotpotato/sweet"),
        path_bytes("coldpotato/sweet"),
    ));

    assert_ok(lfs_remove(lfs, path_bytes("hotpotato")));
    assert_ok(lfs_rename(
        lfs,
        path_bytes("coldpotato"),
        path_bytes("hotpotato"),
    ...
    assert_ok(lfs_remove(lfs, path_bytes("hotpotato/baked")));
    assert_ok(lfs_remove(lfs, path_bytes("hotpotato/fried")));
    assert_ok(lfs_remove(lfs, path_bytes("hotpotato/sweet")));
    assert_ok(lfs_remove(lfs, path_bytes("hotpotato")));
...
fn test_dirs_recursive_remove() {
    ...
    for n in [10usize, 100] {
        ...
        assert_err(
            Error::NotEmpty,
            lfs_remove(lfs, path_bytes("prickly-pear")),
        );
        ...
        assert_ok(lfs_remove(lfs, path_bytes("prickly-pear")));
        assert_ok(lfs_unmount(lfs));
    ...
    for k in 0..N {
        for j in 0..=N {
            ...
            assert_ok(lfs_remove(
                lfs,
                path_bytes(&format!("prickly-pear/cactus{k:03}")),
            ));
            let info = &mut unsafe { core::mem::zeroed::<LfsInfo>() };
...
fn test_dirs_other_errors() {
    ...
    assert_err(
        Error::Exists,
        lfs_mkdir(lfs, path_bytes("burito")),
    );
    ...
    assert_err(
        Error::NoEntry,
        lfs_dir_open(lfs, dir, path_bytes("tomato")),
    );
    let dir = &mut unsafe { core::mem::MaybeUninit::<LfsDir>::zeroed().assume_init() };
    assert_err(
        Error::NotDir,
        lfs_dir_open(lfs, dir, path_bytes("burito")),
    );

    let file = &mut unsafe { core::mem::MaybeUninit::<LfsFile>::zeroed().assume_init() };
    assert_err(
        Error::NoEntry,
        lfs_file_open(lfs, file, path_bytes("tomato"), LFS_O_RDONLY),
    );
    let file = &mut unsafe { core::mem::MaybeUninit::<LfsFile>::zeroed().assume_init() };
    assert_err(
        Error::IsDir,
    ...
    assert_err(
        Error::NoEntry,
        lfs_file_open(lfs, file, path_bytes("tomato"), LFS_O_WRONLY),
    );
    let file = &mut unsafe { core::mem::MaybeUninit::<LfsFile>::zeroed().assume_init() };
    assert_err(
        Error::IsDir,
    ...
    assert_err(
        Error::IsDir,
        lfs_rename(lfs, path_bytes("tacoto"), c"potato"),
    );
    assert_err(
        Error::NotDir,
        lfs_rename(lfs, c"potato", path_bytes("tacoto")),
    );
    ...
    assert_err(
        Error::IsDir,
        lfs_file_open(lfs, file, path_bytes("/"), LFS_O_RDONLY),
    );
    let file = &mut unsafe { core::mem::MaybeUninit::<LfsFile>::zeroed().assume_init() };
    assert_err(
        Error::IsDir,
        lfs_file_open(lfs, file, path_bytes("/"), LFS_O_WRONLY),
    );
    let file = &mut unsafe { core::mem::MaybeUninit::<LfsFile>::zeroed().assume_init() };
    assert_err(
        Error::IsDir,
...
fn test_dirs_seek() {
    ...
    for count in [4usize, 128, 132] {
        ...
        let dir = &mut unsafe { core::mem::MaybeUninit::<LfsDir>::zeroed().assume_init() };
        assert_ok(lfs_dir_open(lfs, dir, path_bytes("child")));
        assert_ok(lfs_dir_rewind(lfs, dir));
