...
use littlefs_rust_core::{error::Error, lfs_type::lfs_type::{LFS_TYPE_DIR, LFS_TYPE_REG}};
...
fn test_move_nop() {
    ...
    assert_ok(lfs_mkdir(lfs, hi.as_ptr()));
    assert_ok(lfs_rename(lfs, hi.as_ptr(), hi.as_ptr()));

    let hi_hi = path_bytes("hi/hi");
    assert_ok(lfs_mkdir(lfs, hi_hi.as_ptr()));
    assert_ok(lfs_rename(lfs, hi_hi.as_ptr(), hi_hi.as_ptr()));

    let hi_hi_hi = path_bytes("hi/hi/hi");
    assert_ok(lfs_mkdir(lfs, hi_hi_hi.as_ptr()));
...
fn test_move_file() {
    ...
    assert_ok(lfs_mkdir(lfs, path_bytes("b").as_ptr()));
    assert_ok(lfs_mkdir(lfs, path_bytes("c").as_ptr()));
...
fn test_move_dir() {
    ...
    assert_ok(lfs_mkdir(lfs, path_bytes("c").as_ptr()));
    assert_ok(lfs_mkdir(lfs, path_bytes("d").as_ptr()));
    assert_ok(lfs_mkdir(lfs, path_bytes("a/hi").as_ptr()));
    assert_ok(lfs_mkdir(
        lfs,
...
fn test_move_state_stealing() {
    ...
    assert_ok(lfs_mount(lfs, &env.config));
    assert_ok(lfs_remove(lfs, path_bytes("b").as_ptr()));
    assert_ok(lfs_remove(lfs, path_bytes("c").as_ptr()));
    assert_ok(lfs_unmount(lfs));
...
fn test_move_create_delete_different() {
    ...
    assert_ok(lfs_mkdir(lfs, path_bytes("dir.1").as_ptr()));
    assert_ok(lfs_mkdir(lfs, path_bytes("dir.2").as_ptr()));
    ...
    let names =
        dir_entry_names(lfs, &env.config, "dir.2").unwrap();
...
fn test_move_dir_corrupt_source() {
    ...
    let hi_names =
        dir_entry_names(lfs, &env.config, "c/hi").unwrap();
...
fn test_move_dir_corrupt_source_dest() {
    ...
    let hi_names =
        dir_entry_names(lfs, &env.config, "a/hi").unwrap();
    assert!(hi_names.contains(&"hola".to_string()));
    assert!(hi_names.contains(&"bonjour".to_string()));
    assert!(hi_names.contains(&"ohayo".to_string()));
