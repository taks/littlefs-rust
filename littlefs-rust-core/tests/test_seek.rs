//! Upstream: tests/test_seek.toml

mod common;

use common::{
    ALPHA, LFS_FILE_MAX, LFS_O_APPEND, LFS_O_CREAT, LFS_O_RDONLY, LFS_O_RDWR, LFS_O_WRONLY,
    LFS_SEEK_CUR, LFS_SEEK_END, LFS_SEEK_SET, LfsConfig,
};
use littlefs_rust_core::{
    Error, Lfs, LfsFile, lfs_file_close, lfs_file_open, lfs_file_read, lfs_file_rewind,
    lfs_file_seek, lfs_file_size, lfs_file_sync, lfs_file_tell, lfs_file_write, lfs_format,
    lfs_mount, lfs_unmount,
};
use littlefs_rust_test_macro::lfs_test;

const KITTY: &[u8] = b"kittycatcat";
const DOGGO: &[u8] = b"doggodogdog";
const HEDGEHOG: &[u8] = b"hedgehoghog";
const PORCUPINE: &[u8] = b"porcupineee";

// ── Upstream Cases ──────────────────────────

/// Upstream: [cases.test_seek_read]
/// defines = [{COUNT=132, SKIP=4}, {COUNT=132, SKIP=128}, ...]
#[lfs_test]
#[case(132, 4)]
#[case(132, 128)]
#[case(200, 10)]
#[case(200, 100)]
#[case(4, 1)]
#[case(4, 2)]
#[tokio::test]
async fn test_seek_read<'a>(cfg: &LfsConfig<'a>, #[case] count: u32, #[case] skip: u32) {
    let lfs = &mut Lfs::default();
    assert_ok!(lfs_format(lfs, cfg).await);
    assert_ok!(lfs_mount(lfs, cfg).await);

    let path = "kitty";
    let file = &mut LfsFile::default();
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_WRONLY | LFS_O_CREAT | LFS_O_APPEND,).await);
    for _ in 0..count {
        let n = lfs_file_write(lfs, file, KITTY).await;
        assert_eq!(n, Ok(KITTY.len() as u32));
    }
    assert_ok!(lfs_file_close(lfs, file).await);
    assert_ok!(lfs_unmount(lfs));

    assert_ok!(lfs_mount(lfs, cfg).await);
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_RDONLY).await);

    let mut buf = [0u8; 32];
    let mut pos: i32 = -1;
    for _ in 0..skip {
        let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
        assert_eq!(n, Ok(KITTY.len() as u32));
        assert_eq!(&buf[..KITTY.len()], KITTY);
        pos = lfs_file_tell(lfs, file) as i32;
    }
    assert!(pos >= 0);

    assert_eq!(
        lfs_file_seek(lfs, file, pos, LFS_SEEK_SET).await,
        Ok(pos as u32)
    );
    let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    assert_eq!(&buf[..KITTY.len()], KITTY);

    assert_ok!(lfs_file_rewind(lfs, file).await);
    let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    assert_eq!(&buf[..KITTY.len()], KITTY);

    assert_eq!(
        lfs_file_seek(lfs, file, 0, LFS_SEEK_CUR).await,
        Ok(KITTY.len() as u32)
    );
    let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    assert_eq!(&buf[..KITTY.len()], KITTY);

    assert_eq!(
        lfs_file_seek(lfs, file, KITTY.len() as i32, LFS_SEEK_CUR).await,
        Ok(3 * KITTY.len() as u32)
    );
    let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    assert_eq!(&buf[..KITTY.len()], KITTY);

    assert_eq!(
        lfs_file_seek(lfs, file, pos, LFS_SEEK_SET).await,
        Ok(pos as u32)
    );
    let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    assert_eq!(&buf[..KITTY.len()], KITTY);

    assert_eq!(
        lfs_file_seek(lfs, file, -(KITTY.len() as i32), LFS_SEEK_CUR).await,
        Ok(pos as u32)
    );
    let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    assert_eq!(&buf[..KITTY.len()], KITTY);

    assert_ok!(lfs_file_seek(lfs, file, -(KITTY.len() as i32), LFS_SEEK_END).await);
    let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    assert_eq!(&buf[..KITTY.len()], KITTY);

    assert_eq!(lfs_file_size(lfs, file), (count * KITTY.len() as u32));
    assert_eq!(
        lfs_file_seek(lfs, file, 0, LFS_SEEK_CUR).await,
        Ok(count * KITTY.len() as u32)
    );

    assert_ok!(lfs_file_close(lfs, file).await);
    assert_ok!(lfs_unmount(lfs));
}

/// Upstream: [cases.test_seek_write]
#[lfs_test]
#[case(132, 4)]
#[case(132, 128)]
#[case(200, 10)]
#[case(200, 100)]
#[case(4, 1)]
#[case(4, 2)]
#[tokio::test]
async fn test_seek_write<'a>(cfg: &LfsConfig<'a>, #[case] count: u32, #[case] skip: u32) {
    let lfs = &mut Lfs::default();
    assert_ok!(lfs_format(lfs, cfg).await);
    assert_ok!(lfs_mount(lfs, cfg).await);

    let path = "kitty";
    let file = &mut LfsFile::default();
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_WRONLY | LFS_O_CREAT | LFS_O_APPEND,).await);
    for _ in 0..count {
        let n = lfs_file_write(lfs, file, KITTY).await;
        assert_eq!(n, Ok(KITTY.len() as u32));
    }
    assert_ok!(lfs_file_close(lfs, file).await);
    assert_ok!(lfs_unmount(lfs));

    assert_ok!(lfs_mount(lfs, cfg).await);
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_RDWR).await);

    let mut buf = [0u8; 32];
    let mut pos: i32 = -1;
    for _ in 0..skip {
        let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
        assert_eq!(n, Ok(KITTY.len() as u32));
        assert_eq!(&buf[..KITTY.len()], KITTY);
        pos = lfs_file_tell(lfs, file) as i32;
    }
    assert!(pos >= 0);

    assert_eq!(
        lfs_file_seek(lfs, file, pos, LFS_SEEK_SET).await,
        Ok(pos as u32)
    );
    let n = lfs_file_write(lfs, file, DOGGO).await;
    assert_eq!(n, Ok(DOGGO.len() as u32));

    assert_eq!(
        lfs_file_seek(lfs, file, pos, LFS_SEEK_SET).await,
        Ok(pos as u32)
    );
    let n = lfs_file_read(lfs, file, &mut buf[..DOGGO.len()]).await;
    assert_eq!(n, Ok(DOGGO.len() as u32));
    assert_eq!(&buf[..DOGGO.len()], DOGGO);

    assert_ok!(lfs_file_rewind(lfs, file).await);
    let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    assert_eq!(&buf[..KITTY.len()], KITTY);

    assert_eq!(
        lfs_file_seek(lfs, file, pos, LFS_SEEK_SET).await,
        Ok(pos as u32)
    );
    let n = lfs_file_read(lfs, file, &mut buf[..DOGGO.len()]).await;
    assert_eq!(n, Ok(DOGGO.len() as u32));
    assert_eq!(&buf[..DOGGO.len()], DOGGO);

    assert_ok!(lfs_file_seek(lfs, file, -(KITTY.len() as i32), LFS_SEEK_END).await);
    let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    assert_eq!(&buf[..KITTY.len()], KITTY);

    assert_eq!(lfs_file_size(lfs, file), (count * KITTY.len() as u32));
    assert_eq!(
        lfs_file_seek(lfs, file, 0, LFS_SEEK_CUR).await,
        Ok(count * KITTY.len() as u32)
    );

    assert_ok!(lfs_file_close(lfs, file).await);
    assert_ok!(lfs_unmount(lfs));
}

/// Upstream: [cases.test_seek_boundary_read]
/// defines.COUNT = 132
#[lfs_test]
#[tokio::test]
async fn test_seek_boundary_read<'a>(cfg: &LfsConfig<'a>) {
    const COUNT: u32 = 132;

    let lfs = &mut Lfs::default();
    assert_ok!(lfs_format(lfs, cfg).await);
    assert_ok!(lfs_mount(lfs, cfg).await);

    let path = "kitty";
    let file = &mut LfsFile::default();
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_WRONLY | LFS_O_CREAT | LFS_O_APPEND,).await);
    for _ in 0..COUNT {
        let n = lfs_file_write(lfs, file, KITTY).await;
        assert_eq!(n, Ok(KITTY.len() as u32));
    }
    assert_ok!(lfs_file_close(lfs, file).await);
    assert_ok!(lfs_unmount(lfs));

    assert_ok!(lfs_mount(lfs, cfg).await);
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_RDONLY).await);

    let size = KITTY.len() as i64;
    let pattern = b"kittycatcatkittycatcat";
    let offsets: [i64; 13] = [
        512,
        1024 - 4,
        512 + 1,
        1024 - 4 + 1,
        512 - 1,
        1024 - 4 - 1,
        512 - size,
        1024 - 4 - size,
        512 - size + 1,
        1024 - 4 - size + 1,
        512 - size - 1,
        1024 - 4 - size - 1,
        size * (COUNT as i64 - 2) - 1,
    ];

    let mut buf = [0u8; 32];
    for off in offsets {
        if off < 0 || off + size > (COUNT as i64 * size) {
            continue;
        }
        assert_eq!(
            lfs_file_seek(lfs, file, off as i32, LFS_SEEK_SET).await,
            Ok(off as u32)
        );
        let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
        assert_eq!(n, Ok(KITTY.len() as u32));
        let base = (off % size) as usize;
        assert_eq!(
            &buf[..KITTY.len()],
            &pattern[base..base + KITTY.len()],
            "off={}",
            off
        );

        let off_after = off + size + 1;
        if off_after >= 0 && off_after + size <= COUNT as i64 * size {
            assert_eq!(
                lfs_file_seek(lfs, file, off_after as i32, LFS_SEEK_SET).await,
                Ok(off_after as u32)
            );
            let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
            assert_eq!(n, Ok(KITTY.len() as u32));
            let base = ((off + 1) % size) as usize;
            assert_eq!(&buf[..KITTY.len()], &pattern[base..base + KITTY.len()]);
        }

        let off_before = off - size - 1;
        if off_before >= 0 {
            assert_eq!(
                lfs_file_seek(lfs, file, off_before as i32, LFS_SEEK_SET).await,
                Ok(off_before as u32)
            );
            let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
            assert_eq!(n, Ok(KITTY.len() as u32));
            let base = ((off - 1).rem_euclid(size)) as usize;
            assert_eq!(&buf[..KITTY.len()], &pattern[base..base + KITTY.len()]);
        }

        assert_eq!(lfs_file_seek(lfs, file, 0, LFS_SEEK_SET).await, Ok(0));
        let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
        assert_eq!(n, Ok(KITTY.len() as u32));
        assert_eq!(&buf[..KITTY.len()], KITTY);

        assert_eq!(
            lfs_file_seek(lfs, file, off as i32, LFS_SEEK_SET).await,
            Ok(off as u32)
        );
        let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
        assert_eq!(n, Ok(KITTY.len() as u32));
        let base = (off % size) as usize;
        assert_eq!(&buf[..KITTY.len()], &pattern[base..base + KITTY.len()]);

        let off_after = off + size + 1;
        if off_after >= 0 && off_after + size <= COUNT as i64 * size {
            assert_eq!(
                lfs_file_seek(lfs, file, off_after as i32, LFS_SEEK_SET).await,
                Ok(off_after as u32)
            );
            let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
            assert_eq!(n, Ok(KITTY.len() as u32));
            let base = ((off + 1) % size) as usize;
            assert_eq!(&buf[..KITTY.len()], &pattern[base..base + KITTY.len()]);
        }

        let off_before = off - size - 1;
        if off_before >= 0 {
            assert_eq!(
                lfs_file_seek(lfs, file, off_before as i32, LFS_SEEK_SET).await,
                Ok(off_before as u32)
            );
            let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
            assert_eq!(n, Ok(KITTY.len() as u32));
            let base = ((off - 1).rem_euclid(size)) as usize;
            assert_eq!(&buf[..KITTY.len()], &pattern[base..base + KITTY.len()]);
        }

        assert_ok!(lfs_file_sync(lfs, file).await);

        assert_eq!(lfs_file_seek(lfs, file, 0, LFS_SEEK_SET).await, Ok(0));
        let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
        assert_eq!(n, Ok(KITTY.len() as u32));
        assert_eq!(&buf[..KITTY.len()], KITTY);

        assert_eq!(
            lfs_file_seek(lfs, file, off as i32, LFS_SEEK_SET).await,
            Ok(off as u32)
        );
        let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
        assert_eq!(n, Ok(KITTY.len() as u32));
        let base = (off % size) as usize;
        assert_eq!(&buf[..KITTY.len()], &pattern[base..base + KITTY.len()]);

        let off_after = off + size + 1;
        if off_after >= 0 && off_after + size <= COUNT as i64 * size {
            assert_eq!(
                lfs_file_seek(lfs, file, off_after as i32, LFS_SEEK_SET).await,
                Ok(off_after as u32)
            );
            let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
            assert_eq!(n, Ok(KITTY.len() as u32));
            let base = ((off + 1) % size) as usize;
            assert_eq!(&buf[..KITTY.len()], &pattern[base..base + KITTY.len()]);
        }

        let off_before = off - size - 1;
        if off_before >= 0 {
            assert_eq!(
                lfs_file_seek(lfs, file, off_before as i32, LFS_SEEK_SET).await,
                Ok(off_before as u32)
            );
            let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
            assert_eq!(n, Ok(KITTY.len() as u32));
            let base = ((off - 1).rem_euclid(size)) as usize;
            assert_eq!(&buf[..KITTY.len()], &pattern[base..base + KITTY.len()]);
        }
    }

    assert_ok!(lfs_file_close(lfs, file).await);
    assert_ok!(lfs_unmount(lfs));
}

/// Upstream: [cases.test_seek_boundary_write]
/// defines.COUNT = 132
#[lfs_test]
fn test_seek_boundary_write(cfg: &LfsConfig) {
    const COUNT: u32 = 132;

    let lfs = &mut Lfs::default();
    assert_ok!(lfs_format(lfs, cfg));
    assert_ok!(lfs_mount(lfs, cfg));

    let path = "kitty";
    let file = &mut LfsFile::default();
    assert_ok!(lfs_file_open(
        lfs,
        file,
        path,
        LFS_O_WRONLY | LFS_O_CREAT | LFS_O_APPEND,
    ));
    for _ in 0..COUNT {
        let n = lfs_file_write(lfs, file, KITTY);
        assert_eq!(n, Ok(KITTY.len() as u32));
    }
    assert_ok!(lfs_file_close(lfs, file));
    assert_ok!(lfs_unmount(lfs));

    assert_ok!(lfs_mount(lfs, cfg));
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_RDWR));

    let size = KITTY.len() as i64;
    let offsets: [i64; 13] = [
        512,
        1024 - 4,
        512 + 1,
        1024 - 4 + 1,
        512 - 1,
        1024 - 4 - 1,
        512 - size,
        1024 - 4 - size,
        512 - size + 1,
        1024 - 4 - size + 1,
        512 - size - 1,
        1024 - 4 - size - 1,
        size * (COUNT as i64 - 2) - 1,
    ];

    let mut buf = [0u8; 32];
    for off in offsets {
        if off < 0 || off + size > COUNT as i64 * size {
            continue;
        }
        assert_eq!(
            lfs_file_seek(lfs, file, off as i32, LFS_SEEK_SET),
            Ok(off as u32)
        );
        let n = lfs_file_write(lfs, file, HEDGEHOG);
        assert_eq!(n, Ok(HEDGEHOG.len() as u32));

        assert_eq!(
            lfs_file_seek(lfs, file, off as i32, LFS_SEEK_SET),
            Ok(off as u32)
        );
        let n = lfs_file_read(lfs, file, &mut buf[..HEDGEHOG.len()]);
        assert_eq!(n, Ok(HEDGEHOG.len() as u32));
        assert_eq!(&buf[..HEDGEHOG.len()], HEDGEHOG);

        assert_eq!(lfs_file_seek(lfs, file, 0, LFS_SEEK_SET), Ok(0));
        let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]);
        assert_eq!(n, Ok(KITTY.len() as u32));
        assert_eq!(&buf[..KITTY.len()], KITTY);

        assert_eq!(
            lfs_file_seek(lfs, file, off as i32, LFS_SEEK_SET),
            Ok(off as u32)
        );
        let n = lfs_file_read(lfs, file, &mut buf[..HEDGEHOG.len()]);
        assert_eq!(n, Ok(HEDGEHOG.len() as u32));
        assert_eq!(&buf[..HEDGEHOG.len()], HEDGEHOG);

        assert_ok!(lfs_file_sync(lfs, file));

        assert_eq!(lfs_file_seek(lfs, file, 0, LFS_SEEK_SET), Ok(0));
        let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]);
        assert_eq!(n, Ok(KITTY.len() as u32));
        assert_eq!(&buf[..KITTY.len()], KITTY);

        assert_eq!(
            lfs_file_seek(lfs, file, off as i32, LFS_SEEK_SET),
            Ok(off as u32)
        );
        let n = lfs_file_read(lfs, file, &mut buf[..HEDGEHOG.len()]);
        assert_eq!(n, Ok(HEDGEHOG.len() as u32));
        assert_eq!(&buf[..HEDGEHOG.len()], HEDGEHOG);
    }

    assert_ok!(lfs_file_close(lfs, file));
    assert_ok!(lfs_unmount(lfs));
}

/// Upstream: [cases.test_seek_out_of_bounds]
#[lfs_test]
#[case(132, 4)]
#[case(132, 128)]
#[case(200, 10)]
#[case(200, 100)]
#[case(4, 2)]
#[case(4, 3)]
#[tokio::test]
async fn test_seek_out_of_bounds<'a>(cfg: &LfsConfig<'a>, #[case] count: u32, #[case] skip: u32) {
    let lfs = &mut Lfs::default();
    assert_ok!(lfs_format(lfs, cfg).await);
    assert_ok!(lfs_mount(lfs, cfg).await);

    let path = "kitty";
    let file = &mut LfsFile::default();
    assert_ok!(lfs_file_open(
        lfs,
        file,
        path,
        LFS_O_WRONLY | LFS_O_CREAT | LFS_O_APPEND,
    ));
    for _ in 0..count {
        let n = lfs_file_write(lfs, file, KITTY).await;
        assert_eq!(n, Ok(KITTY.len() as u32));
    }
    assert_ok!(lfs_file_close(lfs, file));
    assert_ok!(lfs_unmount(lfs));

    assert_ok!(lfs_mount(lfs, cfg).await);
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_RDWR).await);

    let size = KITTY.len() as i64;
    let hole_offset = (count as i64 + skip as i64) * size;

    assert_eq!(lfs_file_size(lfs, file), (count * KITTY.len() as u32));
    assert_eq!(
        lfs_file_seek(lfs, file, hole_offset as i32, LFS_SEEK_SET,).await,
        Ok(hole_offset as u32)
    );
    let mut buf = [0u8; 32];
    let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
    assert_eq!(n, Ok(0));

    let n = lfs_file_write(lfs, file, PORCUPINE).await;
    assert_eq!(n, Ok(PORCUPINE.len() as u32));

    assert_eq!(
        lfs_file_seek(lfs, file, hole_offset as i32, LFS_SEEK_SET,).await,
        Ok(hole_offset as u32)
    );
    let n = lfs_file_read(lfs, file, &mut buf[..PORCUPINE.len()]).await;
    assert_eq!(n, Ok(PORCUPINE.len() as u32));
    assert_eq!(&buf[..PORCUPINE.len()], PORCUPINE);

    assert_eq!(
        lfs_file_seek(lfs, file, (count as i32) * (size as i32), LFS_SEEK_SET,).await,
        Ok(count * (size as u32))
    );
    let n = lfs_file_read(lfs, file, &mut buf[..KITTY.len()]).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    assert!(
        buf[..KITTY.len()].iter().all(|&b| b == 0),
        "hole should be zeros, got {:?}",
        &buf[..KITTY.len()]
    );

    // After read at count*size we're at (count+1)*size
    assert_eq!(
        lfs_file_seek(lfs, file, -(hole_offset as i32), LFS_SEEK_CUR,).await,
        Err(Error::Invalid)
    );
    assert_eq!(lfs_file_tell(lfs, file), (count + 1) * (size as u32));

    assert_eq!(
        lfs_file_seek(
            lfs,
            file,
            -((count as i32 + 2 * skip as i32) * (size as i32)),
            LFS_SEEK_END,
        ).await,
        Err(Error::Invalid)
    );
    assert_eq!(lfs_file_tell(lfs, file), (count + 1) * (size as u32));

    assert_ok!(lfs_file_close(lfs, file).await);
    assert_ok!(lfs_unmount(lfs));
}

/// Upstream: [cases.test_seek_inline_write]
/// defines.SIZE = [2, 4, 128, 132]
#[lfs_test]
#[case(2)]
#[case(4)]
#[case(128)]
#[case(132)]
#[tokio::test]
async fn test_seek_inline_write<'a>(cfg: &LfsConfig<'a>, #[case] size: u32) {
    let lfs = &mut Lfs::default();
    assert_ok!(lfs_format(lfs, cfg).await);
    assert_ok!(lfs_mount(lfs, cfg).await);

    let path = "tinykitty";
    let file = &mut LfsFile::default();
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_RDWR | LFS_O_CREAT).await);

    let mut j = 0usize;
    let mut k = 0usize;

    for i in 0..size {
        let c = ALPHA[j % 26];
        let n = lfs_file_write(lfs, file, &[c]).await;
        assert_eq!(n, Ok(1));
        assert_eq!(lfs_file_tell(lfs, file), (i + 1));
        assert_eq!(lfs_file_size(lfs, file), (i + 1));
        j += 1;
    }

    assert_eq!(lfs_file_seek(lfs, file, 0, LFS_SEEK_SET).await, Ok(0));
    assert_eq!(lfs_file_tell(lfs, file), 0);
    assert_eq!(lfs_file_size(lfs, file), size);

    let mut c = [0u8; 1];
    for _ in 0..size {
        let n = lfs_file_read(lfs, file, &mut c).await;
        assert_eq!(n, Ok(1));
        assert_eq!(c[0], ALPHA[k % 26]);
        k += 1;
    }

    assert_ok!(lfs_file_sync(lfs, file).await);
    assert_eq!(lfs_file_tell(lfs, file), size);
    assert_eq!(lfs_file_size(lfs, file), size);

    assert_eq!(lfs_file_seek(lfs, file, 0, LFS_SEEK_SET).await, Ok(0));

    for i in 0..size {
        let c = ALPHA[j % 26];
        let n = lfs_file_write(lfs, file, &[c]).await;
        assert_eq!(n, Ok(1));
        assert_eq!(lfs_file_tell(lfs, file), (i + 1));
        assert_eq!(lfs_file_size(lfs, file), size);
        assert_ok!(lfs_file_sync(lfs, file).await);
        assert_eq!(lfs_file_tell(lfs, file), (i + 1));
        assert_eq!(lfs_file_size(lfs, file), size);

        if i < size - 2 {
            let mut buf3 = [0u8; 3];
            assert_eq!(lfs_file_seek(lfs, file, -1, LFS_SEEK_CUR).await, Ok(i));
            let n = lfs_file_read(lfs, file, &mut buf3).await;
            assert_eq!(n, Ok(3));
            assert_eq!(lfs_file_tell(lfs, file), (i + 3));
            assert_eq!(lfs_file_size(lfs, file), size);
            assert_eq!(
                lfs_file_seek(lfs, file, (i + 1) as i32, LFS_SEEK_SET).await,
                Ok(i + 1)
            );
            assert_eq!(lfs_file_tell(lfs, file), (i + 1));
            assert_eq!(lfs_file_size(lfs, file), size);
        }
        j += 1;
    }

    assert_eq!(lfs_file_seek(lfs, file, 0, LFS_SEEK_SET).await, Ok(0));
    assert_eq!(lfs_file_tell(lfs, file), 0);
    assert_eq!(lfs_file_size(lfs, file), size);

    let mut c = [0u8; 1];
    for _ in 0..size {
        let n = lfs_file_read(lfs, file, &mut c).await;
        assert_eq!(n, Ok(1));
        assert_eq!(c[0], ALPHA[k % 26]);
        k += 1;
    }

    assert_ok!(lfs_file_sync(lfs, file).await);
    assert_eq!(lfs_file_tell(lfs, file), size);
    assert_eq!(lfs_file_size(lfs, file), size);

    assert_ok!(lfs_file_close(lfs, file).await);
    assert_ok!(lfs_unmount(lfs));
}

/// Upstream: [cases.test_seek_reentrant_write]
/// defines.COUNT = [4, 64, 128], POWERLOSS_BEHAVIOR = [NOOP, OOO]
#[lfs_test]
#[case(4)]
#[case(64)]
#[case(128)]
#[cfg(feature = "slow_tests")]
fn test_seek_reentrant_write(
    cfg: &LfsConfig,
    #[values(false, true)] reentrant: bool,
    #[case] count: u32,
) {
    let lfs = &mut Lfs::default();

    let err = littlefs_rust_core::lfs_mount(lfs, cfg);
    if err.is_err() {
        assert_ok!(littlefs_rust_core::lfs_format(lfs, cfg));
        assert_ok!(littlefs_rust_core::lfs_mount(lfs, cfg));
    }

    let path = "kitty";
    let file = &mut LfsFile::default();
    let mut buf = [0u8; 32];

    let open_err = littlefs_rust_core::lfs_file_open(lfs, file, path, LFS_O_RDONLY);
    if open_err.is_ok() {
        let sz = littlefs_rust_core::lfs_file_size(lfs, file);
        if sz != 0 {
            assert_eq!(sz, count * 11);
            for _ in 0..count {
                assert_eq!(
                    littlefs_rust_core::lfs_file_read(lfs, file, &mut buf[..11]),
                    Ok(11)
                );
                assert!(
                    &buf[..11] == KITTY || &buf[..11] == DOGGO,
                    "unexpected content"
                );
            }
        }
        assert_ok!(littlefs_rust_core::lfs_file_close(lfs, file));
    } else {
        assert_eq!(open_err, Err(Error::NoEntry));
    }

    assert_ok!(littlefs_rust_core::lfs_file_open(
        lfs,
        file,
        path,
        LFS_O_WRONLY | LFS_O_CREAT
    ));

    if littlefs_rust_core::lfs_file_size(lfs, file) == 0 {
        for _ in 0..count {
            assert_eq!(
                littlefs_rust_core::lfs_file_write(lfs, file, KITTY),
                Ok(KITTY.len() as u32)
            );
        }
    }
    assert_ok!(littlefs_rust_core::lfs_file_close(lfs, file));

    assert_ok!(littlefs_rust_core::lfs_file_open(
        lfs, file, path, LFS_O_RDWR
    ));

    assert_eq!(littlefs_rust_core::lfs_file_size(lfs, file), count * 11);

    let mut off: u32 = 0;
    for _ in 0..count {
        off = (5 * off + 1) % count;
        let pos = off * 11;
        assert_eq!(
            littlefs_rust_core::lfs_file_seek(lfs, file, pos as i32, LFS_SEEK_SET),
            Ok(pos)
        );

        assert_eq!(
            littlefs_rust_core::lfs_file_read(lfs, file, &mut buf[..11]),
            Ok(11)
        );

        assert!(&buf[..11] == KITTY || &buf[..11] == DOGGO);
        if &buf[..11] != DOGGO {
            assert_eq!(
                littlefs_rust_core::lfs_file_seek(lfs, file, pos as i32, LFS_SEEK_SET),
                Ok(pos)
            );

            assert_eq!(
                littlefs_rust_core::lfs_file_write(lfs, file, DOGGO),
                Ok(DOGGO.len() as u32)
            );

            assert_eq!(
                littlefs_rust_core::lfs_file_seek(lfs, file, pos as i32, LFS_SEEK_SET),
                Ok(pos)
            );

            assert_eq!(
                littlefs_rust_core::lfs_file_read(lfs, file, &mut buf[..11]),
                Ok(11)
            );
            assert_eq!(&buf[..11], DOGGO);
            assert_ok!(littlefs_rust_core::lfs_file_sync(lfs, file));
            assert_eq!(
                littlefs_rust_core::lfs_file_seek(lfs, file, pos as i32, LFS_SEEK_SET),
                Ok(pos)
            );

            assert_eq!(
                littlefs_rust_core::lfs_file_read(lfs, file, &mut buf[..11]),
                Ok(11)
            );
            assert_eq!(&buf[..11], DOGGO);
        }
    }

    assert_ok!(littlefs_rust_core::lfs_file_close(lfs, file));

    assert_ok!(littlefs_rust_core::lfs_file_open(
        lfs, file, path, LFS_O_RDWR
    ));
    assert_eq!(littlefs_rust_core::lfs_file_size(lfs, file), count * 11);
    for _ in 0..count {
        assert_eq!(
            littlefs_rust_core::lfs_file_read(lfs, file, &mut buf[..11]),
            Ok(11)
        );

        assert_eq!(&buf[..11], DOGGO);
    }
    assert_ok!(littlefs_rust_core::lfs_file_close(lfs, file));

    assert_ok!(littlefs_rust_core::lfs_unmount(lfs));
}

/// Upstream: [cases.test_seek_filemax]
#[lfs_test]
#[tokio::test]
async fn test_seek_filemax<'a>(cfg: &LfsConfig<'a>) {
    let lfs = &mut Lfs::default();
    assert_ok!(lfs_format(lfs, cfg).await);
    assert_ok!(lfs_mount(lfs, cfg).await);

    let path = "kitty";
    let file = &mut LfsFile::default();
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_WRONLY | LFS_O_CREAT | LFS_O_APPEND,).await);
    let n = lfs_file_write(lfs, file, KITTY).await;
    assert_eq!(n, Ok(KITTY.len() as u32));

    assert_eq!(
        lfs_file_seek(lfs, file, LFS_FILE_MAX, LFS_SEEK_SET).await,
        Ok(LFS_FILE_MAX as u32)
    );

    assert_eq!(
        lfs_file_seek(lfs, file, 0, LFS_SEEK_CUR).await,
        Ok(LFS_FILE_MAX as u32)
    );

    assert_eq!(
        lfs_file_seek(lfs, file, 10, LFS_SEEK_END).await,
        Ok(KITTY.len() as u32 + 10)
    );

    assert_ok!(lfs_file_close(lfs, file).await);
    assert_ok!(lfs_unmount(lfs));
}

/// Upstream: [cases.test_seek_underflow]
#[lfs_test]
#[tokio::test]
async fn test_seek_underflow<'a>(cfg: &LfsConfig<'a>) {
    let lfs = &mut Lfs::default();
    assert_ok!(lfs_format(lfs, cfg).await);
    assert_ok!(lfs_mount(lfs, cfg).await);

    let path = "kitty";
    let file = &mut LfsFile::default();
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_WRONLY | LFS_O_CREAT | LFS_O_APPEND,).await);
    let n = lfs_file_write(lfs, file, KITTY).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    let size = KITTY.len() as i32;

    assert_eq!(
        lfs_file_seek(lfs, file, -(size + 10), LFS_SEEK_CUR).await,
        Err(Error::Invalid)
    );
    assert_eq!(
        lfs_file_seek(lfs, file, -LFS_FILE_MAX, LFS_SEEK_CUR).await,
        Err(Error::Invalid)
    );
    assert_eq!(
        lfs_file_seek(
            lfs,
            file,
            i32::MIN, // -(size + LFS_FILE_MAX) overflows; use MIN to trigger underflow
            LFS_SEEK_CUR,
        )
        .await,
        Err(Error::Invalid)
    );

    assert_eq!(
        lfs_file_seek(lfs, file, -(size + 10), LFS_SEEK_END).await,
        Err(Error::Invalid)
    );
    assert_eq!(
        lfs_file_seek(lfs, file, -LFS_FILE_MAX, LFS_SEEK_END).await,
        Err(Error::Invalid)
    );
    assert_eq!(
        lfs_file_seek(
            lfs,
            file,
            i32::MIN, // -(size + LFS_FILE_MAX) overflows; use MIN
            LFS_SEEK_END,
        )
        .await,
        Err(Error::Invalid)
    );

    assert_eq!(lfs_file_tell(lfs, file), size as u32);

    assert_ok!(lfs_file_close(lfs, file).await);
    assert_ok!(lfs_unmount(lfs));
}

/// Upstream: [cases.test_seek_overflow]
#[lfs_test]
#[tokio::test]
async fn test_seek_overflow<'a>(cfg: &LfsConfig<'a>) {
    let lfs = &mut Lfs::default();
    assert_ok!(lfs_format(lfs, cfg).await);
    assert_ok!(lfs_mount(lfs, cfg).await);

    let path = "kitty";
    let file = &mut LfsFile::default();
    assert_ok!(lfs_file_open(lfs, file, path, LFS_O_WRONLY | LFS_O_CREAT | LFS_O_APPEND,).await);
    let n = lfs_file_write(lfs, file, KITTY).await;
    assert_eq!(n, Ok(KITTY.len() as u32));
    let size = KITTY.len() as u32;

    assert_eq!(
        lfs_file_seek(lfs, file, LFS_FILE_MAX, LFS_SEEK_SET).await,
        Ok(LFS_FILE_MAX as u32)
    );

    assert_eq!(
        lfs_file_seek(lfs, file, 10, LFS_SEEK_CUR).await,
        Err(Error::Invalid)
    );
    assert_eq!(
        lfs_file_seek(lfs, file, LFS_FILE_MAX, LFS_SEEK_CUR).await,
        Err(Error::Invalid)
    );

    assert_eq!(
        lfs_file_seek(lfs, file, LFS_FILE_MAX.wrapping_add(10), LFS_SEEK_SET,).await,
        Err(Error::Invalid)
    );
    assert_eq!(
        lfs_file_seek(
            lfs,
            file,
            LFS_FILE_MAX.wrapping_add(LFS_FILE_MAX),
            LFS_SEEK_SET,
        )
        .await,
        Err(Error::Invalid)
    );

    assert_eq!(
        lfs_file_seek(
            lfs,
            file,
            LFS_FILE_MAX.wrapping_sub(size as i32).wrapping_add(10),
            LFS_SEEK_END,
        )
        .await,
        Err(Error::Invalid)
    );
    assert_eq!(
        lfs_file_seek(
            lfs,
            file,
            LFS_FILE_MAX
                .wrapping_sub(size as i32)
                .wrapping_add(LFS_FILE_MAX),
            LFS_SEEK_END,
        )
        .await,
        Err(Error::Invalid)
    );

    assert_eq!(lfs_file_tell(lfs, file), LFS_FILE_MAX as u32);

    assert_ok!(lfs_file_close(lfs, file).await);
    assert_ok!(lfs_unmount(lfs));
}
