...
fn test_paths_trailing_dotdots(#[case] dir_mode: bool) {
    ...
    if dir_mode {
        assert_err(
            Error::Exists,
            lfs_mkdir(lfs, ("coffee/drip/..")),
        );
        assert_err(
            Error::Exists,
            lfs_mkdir(lfs, ("coffee/coldbrew/../..")),
        );
        assert_err(
            Error::Invalid,
            lfs_mkdir(lfs, ("coffee/turkish/../../..")),
        );
        assert_err(
            Error::Invalid,
...
fn test_paths_magic_conflict(#[case] dir_mode: bool) {
    ...
    assert_err(
        Error::NoEntry,
        lfs_stat(lfs, ("coffee"), info),
    );
    assert_ok(lfs_remove(lfs, ("littlefs")));
    assert_err(
        Error::NoEntry,
        lfs_stat(lfs, ("littlefs"), info),
    );
    assert_ok(lfs_unmount(lfs));
}
