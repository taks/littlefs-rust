...
impl SuperblockSnapshot {
    ...
    pub fn capture<S>(config: &LfsConfig<S>, root_pair: [u32; 2]) -> Result<Self, Error> {
        ...
        unsafe { config.context.unwrap().as_mut().read(0, 0, &mut block0) }?;
        unsafe { config.context.unwrap().as_mut().read(1, 0, &mut block1) }?;
