...
/// ```
#[inline(always)]
pub fn lfs_cache_drop<S>(_lfs: &Lfs<S>, rcache: &mut LfsCache) {
...
/// ```
#[inline(always)]
pub fn lfs_cache_zero<S>(_lfs: &Lfs<S>, pcache: &mut LfsCache) {
    unsafe {
...
pub async fn lfs_bd_read<S: Storage>(
    ...
    while !data.is_empty() {
        ...
        let err = unsafe { lfs.cfg.as_ref().context.unwrap_unchecked().as_mut() }
            .read(rcache.block, rcache.off as u32, data_)
            .await;
        if err.is_err() {
...
pub async fn lfs_bd_flush<S: Storage>(
    ...
    if pcache.block != crate::types::LFS_BLOCK_NULL && pcache.block != LFS_BLOCK_INLINE {
        ...
        let err = unsafe { lfs.cfg.as_ref().context.unwrap_unchecked().as_mut() }
            .write(pcache.block, pcache.off as u32, data_)
            .await;
        crate::lfs_pass_err!(err, "bd_prog block={} -> CORRUPT", pcache.block)?;

        if validate {
            lfs_cache_drop(lfs, rcache);
            let res = lfs_bd_cmp(lfs, None, rcache, diff, pcache.block, pcache.off, data_).await;
            res?;
            if let Ok(res) = res
