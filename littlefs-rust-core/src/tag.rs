...
impl Tag {
    ...
    /// ```
    #[inline(always)]
    pub fn mktag(type_: u32, id: u32, size: u32) -> Self {
        Self(((type_ as lfs_tag_t) << 20) | ((id as lfs_tag_t) << 10) | (size as lfs_tag_t))
    }
    ...
    /// ```
    #[inline(always)]
    pub fn mktag_if(cond: bool, type_: u32, id: u32, size: u32) -> Self {
        if cond {
            Self::mktag(type_, id, size)
        } else {
            Self::mktag(crate::lfs_type::lfs_type::LFS_FROM_NOOP, 0, 0)
        }
    }
    ...
    /// ```
    #[inline(always)]
    pub fn lfs_tag_splice(self) -> i8 {
        self.lfs_tag_chunk() as i8
    }
    ...
    /// ```
    #[inline(always)]
    pub fn dsize(self) -> lfs_size_t {
        let size = if self.isdelete() {
            Tag(self.0.wrapping_add(1)).size()
        } else {
            self.size()
        };
        4 + size // sizeof(tag)
    ...
    #[inline(always)]
    pub fn wrapping_add(self, rhs: Self) -> Tag {
        Tag(self.0.wrapping_add(rhs.0))
    }
}
